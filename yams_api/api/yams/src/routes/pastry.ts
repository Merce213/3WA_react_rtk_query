...
router.get(
	...
	async (req: CustomRequest, res: Response) => {
		...
		const pastries: Pastry[] | undefined = req.locals?.pastries;
		const pastrie: Pastry | undefined = pastries?.find((p) =>
			p.name.match(re)
		);
...
router.get(
	"/pastries/:offset?/:limit",
	async (req: CustomRequest, res: Response) => {
		const offset: number = parseInt(req.params.offset);
		const limit: number = parseInt(req.params.limit);
		const pastries: Pastry[] | undefined = req.locals?.pastries;

		const p: Pastry[] | undefined = limit
			? pastries?.slice(offset).slice(0, limit)
			: pastries?.slice(offset);
